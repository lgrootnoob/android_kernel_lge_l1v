...
static int mipi_dsi_panel_power(int on)
{
	...
	struct pm_gpio gpio43_param = {
		.direction = PM_GPIO_DIR_OUT,
		.output_buffer = PM_GPIO_OUT_BUF_CMOS,
		.output_value = 1, /*2012-1226 skip LCD init for L1v*/
		.pull = PM_GPIO_PULL_NO,
		.vin_sel = 2,
		.out_strength = PM_GPIO_STRENGTH_HIGH,
		.function = PM_GPIO_FUNC_PAIRED,
	...
	if (on) {
		...
		#endif  /*2012-1226 skip LCD init for L1v*/
		
	} else {

            printk(KERN_ERR "mipi_dsi_panel_power OFF\n");  
        #if defined(CONFIG_MACH_MSM8960_L1v) /*2012-1226 skip LCD init for L1v*/
	   if(system_state != SYSTEM_BOOTING)
		gpio_set_value_cansleep(gpio43, 0);
	#endif
...
//	.num_mdp_clk = ARRAY_SIZE(mdp_core_clk_rate_table),
	...
	.cont_splash_enabled = 0x01,  /*2012-1226 skip LCD init for L1v*/
#else
...
//#define Dynamic_High    1
//#define Dynamic_Low    1
//#define Static_BlueH 	1
//#define Static_BlueM_RedM	1

//#define Static_BlueH_RedM 	1
...
//#define Color_11201433
//#define Color_11231316
//#define Color_11271832

//#define Color_11281655_1
//#define Color_11281655_2
//#define Color_11281655_3
//#define Color_11281655_4

//#define Color_11291343_1 
//#define Color_11291343_2

//#define Color_1204_01
//#define Color_1204_02
//#define Color_1204_03
//#define Color_1204_04
//#define Color_1204_05

#define Color_1204_06

//#define gamma_1205_01
...
static char Set_POWER2 [20] = {0xB1, 0x00, 0x00, 0x04, 0xE9, 0x9C, 0x10, 0x11, 0x92, 0x0F, 0x2B, 0x33, 0x22, 0x22, 0x41, 0x00, 0x3A, 0xF7, 0x20, 0x00};

static char Set_DISPLAY[ 8] = {0xB2, 0x00, 0x00, 0x78, 0x05, 0x03, 0x00, 0xF0};

static char Set_VCOM   [ 3] = {0xB6, 0x00, 0xB5};
...
#if defined(gamma_1205_01)
static char Set_DGC_LUT[128]= {};

static char Set_GAMMA  [35] = {0xE0, 0x05, 0x15, 0x19, 0x35, 0x3F, 0x3F, 0x24, 0x3F, 0x04, 0x0C, 		0x0D, 0x11, 0x14, 0x12, 0x14, 0x0F, 0x1A, 0x05, 0x15, 0x19, 
                                     						0x35, 0x3F, 0x3F, 0x24, 0x3F, 0x04, 0x0C, 0x0D, 0x11, 0x14, 		0x12, 0x14, 0x0F, 0x1A};
...
static char Set_DGC_LUT[128]= {0xC1, 0x01, 0x00, 0x07, 0x1C, 0x2A, 0x34, 0x3F, 0x49, 0x52, 0x5B, 0x63, 0x6A, 0x71, 0x78, 0x7F, 0x85, 0x8B, 0x91, 0x97, 0x9D, 
									 ...
									 0x8B, 0x91, 0x97, 0x9D, 0xA3, 0xA8, 0xAF, 0xB5, 0xBE, 0xC6, 0xC9, 0xCD, 0xD7, 0xE1, 0xE6, 0xF1, 0xF7, 0xFF, 0x34, 0xCB, 
...
#endif
#else /*0621 gamma*/
static char Set_DGC_LUT[128]= {};
static char Set_GAMMA  [35] = {0xE0, 0x05, 0x11, 0x14, 0x37, 0x3F, 0x3F, 0x20, 0x4F, 0x05, 0x0E, 0x0D, 0x12, 0x14, 0x12, 0x14, 0x1D, 0x1C, 0x05, 0x11, 0x14, 
                                     0x37, 0x3F, 0x3F, 0x20, 0x4F, 0x05, 0x0E, 0x0D, 0x12, 0x14, 0x12, 0x14, 0x1D, 0x1C};
#endif

#if defined (Dynamic_High)
static char Set_CE4    [2]  = {0xE4, 0x03};
static char Set_CE6    [2]  = {0xE6, 0x01};
#elif defined (Dynamic_Low)
static char Set_CE4    [2]  = {0xE4, 0x01};
static char Set_CE6    [2]  = {0xE6, 0x01};
#elif defined (Static_BlueH)
	static char Set_CE3    [2]  = {0xE3, 0x01};
	static char Set_CE5    [19] = {0xE5, 0x00, 0x00, 0x1F, 0x04, 0x02, 0x00, 0x80, 0x20, 0x10, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E};
	static char Set_CE2    [18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x10, 0x11, 0x15, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x10, 0x10};	
#elif defined (Static_BlueM_RedM)
    static char Set_CE3    [2]  = {0xE3, 0x01};
	static char Set_CE5    [19] = {0xE5, 0x00, 0x04, 0x16, 0x04, 0x02, 0x00, 0x80, 0x20, 0x10, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E};
	static char Set_CE2    [18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x10, 0x0F, 0x11, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x10, 0x10};	
#elif defined (Static_BlueH_RedM)
    static char Set_CE3    [2]  = {0xE3, 0x01};
	static char Set_CE5    [19] = {0xE5, 0x00, 0x0C, 0x1F, 0x07, 0x02, 0x00, 0x80, 0x20, 0x10, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E};
	static char Set_CE2    [18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x0A, 0x09, 0x09, 0x10, 0x10, 0x20, 0x20, 0x20, 0x20, 0x10, 0x10, 0x10};
#elif defined (Green1) 
	static char Set_CE3	[2]  = {0xE3, 0x01}; 
	static char Set_CE5 	[19] = {0xE5, 0x00, 0x0C, 0x1F, 0x07, 0x02, 0x00, 0x80, 0x20, 0x10, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E}; 
	static char Set_CE2 	[18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x0A, 0x09, 0x09, 0x10, 0x08, 0x10, 0x20, 0x20, 0x20, 0x10, 0x10, 0x10}; 
#elif defined (Green2) 
	static char Set_CE3	[2]  = {0xE3, 0x01}; 
	static char Set_CE5 	[19] = {0xE5, 0x00, 0x0C, 0x1F, 0x07, 0x02, 0x00, 0x80, 0x20, 0x10, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E}; 
	static char Set_CE2 	[18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x0A, 0x09, 0x09, 0x10, 0x04, 0x08, 0x20, 0x20, 0x20, 0x10, 0x10, 0x10}; 
#elif defined (Green3)
	static char Set_CE3 [2] = {0xE3, 0x01};
	static char Set_CE5 [19] = {0xE5, 0x00, 0x0C, 0x1F, 0x07, 0x02, 0x00, 0x80, 0x20, 0x10, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E};
	static char Set_CE2 [18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x11, 0x11, 0x11, 0x10, 0x10, 0x20, 0x20, 0x20, 0x20, 0x10, 0x10, 0x10};	
#elif defined (GreenLow)
	static char Set_CE3 [2] = {0xE3, 0x01};
	static char Set_CE5 [19] = {0xE5, 0x00, 0x0C, 0x1F, 0x07, 0x02, 0x00, 0x80, 0x20, 0x10, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E};
	static char Set_CE2 [18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x10, 0x10, 0x0F, 0x04, 0x04, 0x08, 0x20, 0x20, 0x20, 0x10, 0x10, 0x10}; 
#elif defined (Color_11201433)
	static char Set_CE3 [2] = {0xE3, 0x01};
	static char Set_CE5 [19] = {0xE5, 0x00, 0x0C, 0x1F, 0x07, 0x02, 0x00, 0x80, 0x20, 0x10, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E};
	static char Set_CE2 [18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x0A, 0x10, 0x0F, 0x04, 0x04, 0x08, 0x20, 0x20, 0x20, 0x10, 0x10, 0x10}; 
#elif defined (Color_11231316) 
	 static char Set_CE3 [2] = {0xE3, 0x01}; 
	 static char Set_CE5 [19] = {0xE5, 0x00, 0x0C, 0x1F, 0x07, 0x02, 0x00, 0x80, 0x20, 0x40, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E}; 
	 static char Set_CE2 [18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x0A, 0x10, 0x0F, 0x04, 0x04, 0x08, 0x20, 0x20, 0x20, 0x10, 0x10, 0x10}; 
#elif defined (Color_11271832) 
	static char Set_CE3 [2] = {0xE3, 0x01}; 
	static char Set_CE5 [19] = {0xE5, 0x00, 0x0C, 0x1F, 0x07, 0x02, 0x00, 0x80, 0x20, 0x00, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E}; 
	static char Set_CE2 [18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x0A, 0x10, 0x0F, 0x04, 0x04, 0x08, 0x20, 0x20, 0x20, 0x10, 0x10, 0x10}; 

#elif defined (Color_1204_01)  
static char Set_CE3 [2] = {0xE3, 0x01};
static char Set_CE5 [19] = {0xE5, 0x00, 0x0C, 0x1F, 0x07, 0x02, 0x00, 0x08, 0x20, 0x00, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E};
static char Set_CE2 [18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10}; 

#elif defined (Color_1204_02)  
static char Set_CE3 [2] = {0xE3, 0x01};
static char Set_CE5 [19] = {0xE5, 0x00, 0x1C, 0x17, 0x07, 0x07, 0x00, 0x08, 0x20, 0x00, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E};
static char Set_CE2 [18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10};

#elif defined (Color_1204_03)  
static char Set_CE3 [2] = {0xE3, 0x01}; 
static char Set_CE5 [19] = {0xE5, 0x00, 0x1C, 0x17, 0x07, 0x07, 0x00, 0x08, 0x20, 0x00, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E}; 
static char Set_CE2 [18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x10, 0x10, 0x07, 0x10, 0x10, 0x10, 0x1E, 0x1E, 0x1C, 0x1C, 0x10, 0x10}; 

#elif defined (Color_1204_04)  
static char Set_CE3 [2] = {0xE3, 0x01};
static char Set_CE5 [19] = {0xE5, 0x00, 0x0C, 0x1F, 0x07, 0x04, 0x00, 0x08, 0x20, 0x00, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E};
static char Set_CE2 [18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x10, 0x10, 0x0E, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10};

#elif defined (Color_1204_05)  
static char Set_CE3 [2] = {0xE3, 0x01};
static char Set_CE5 [19] = {0xE5, 0x00, 0x0C, 0x1F, 0x07, 0x04, 0x00, 0x08, 0x20, 0x00, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E};
static char Set_CE2 [18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10};

#elif defined (Color_1204_06)  
static char Set_CE3 [2] = {0xE3, 0x01};
static char Set_CE5 [19] = {0xE5, 0x00, 0x0C, 0x1F, 0x07, 0x04, 0x00, 0x08, 0x20, 0x00, 0x10, 0x00, 0x00, 0x08, 0x06, 0x04, 0x00, 0x80, 0x0E};
static char Set_CE2 [18] = {0xE2, 0x40, 0x00, 0x00, 0x05, 0x05, 0x10, 0x10, 0x10, 0x04, 0x04, 0x08, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10};

#endif


#ifdef LGIT_IEF_SWITCH
static char Set_CE3_off    [2]  = {0xE3, 0x00};
static char Set_CE2_off    [2]  = {0xE2, 0x00};
#endif
...
/*Color Engine*/
        {DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(Set_CE3), Set_CE3},
	{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(Set_CE5), Set_CE5},
	{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(Set_CE2), Set_CE2},
...
#ifdef LGIT_IEF_SWITCH
static struct dsi_cmd_desc lgd_ief_off_set[] = {	
	{DTYPE_DCS_LWRITE, 1, 0, 0, 10, sizeof(Set_CE3_off), Set_CE3_off},
	{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(Set_CE2_off), Set_CE2_off},	
};

static struct dsi_cmd_desc lgd_ief_on_set[] = {
   {DTYPE_DCS_LWRITE, 1, 0, 0, 10, sizeof(Set_CE3), Set_CE3},
	{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(Set_CE5), Set_CE5},
	{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(Set_CE2), Set_CE2},
};
#endif
...
static struct msm_panel_common_pdata mipi_lgd_pdata = {
	...
	#ifdef LGIT_IEF_SWITCH
	.power_off_set_ief = lgd_ief_off_set,	
	.power_on_set_ief = lgd_ief_on_set,
	.power_off_set_ief_size = ARRAY_SIZE(lgd_ief_off_set),	
	.power_on_set_ief_size =ARRAY_SIZE(lgd_ief_on_set),
	#endif
#endif
#if defined (CONFIG_LGE_BACKLIGHT_LM3530)
   #if defined(CONFIG_MACH_MSM8960_L1v)
...
{
	#if defined(CONFIG_MACH_MSM8960_L1v) /*not used*/
	panel_gpiomux_init();
	#endif
