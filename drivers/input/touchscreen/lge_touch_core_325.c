...
bool block_interrupt;
	...
	#if defined(CONFIG_MACH_MSM8960_FX1S) || defined(CONFIG_MACH_MSM8960_FX1SU)
		bool block_interrupt;
	#endif
#endif
...
#if defined(CONFIG_MACH_MSM8960_L1)
if((cnt-1) >= 0) {
							TOUCH_INFO_MSG("%d finger pressed : <id:%d> <cnt%d> x[%4d] y[%4d] p[%3d](Original)\n",
									ts->ts_data.total_num-1, ts->ts_data.curr_data[cnt-1].id, cnt-1,
									ts->ts_data.curr_data[cnt-1].x_position,
									ts->ts_data.curr_data[cnt-1].y_position,
									ts->ts_data.curr_data[cnt-1].pressure);
					}
#endif
						}
...
if((cnt-1) >= 0) {
#endif
							if ( 1<= ts->ts_data.prev_total_num && 1<= ts->ts_data.total_num && jitter_sub(
#if defined(CONFIG_MACH_MSM8960_L1)
ts->ts_data.curr_data[cnt-1].x_position,
#else
ts_prev_finger_press_data.x_position,
#endif
ts->ts_data.curr_data[cnt].x_position)<10
&& jitter_sub(
#if defined(CONFIG_MACH_MSM8960_L1)
ts->ts_data.curr_data[cnt-1].y_position,
#else
ts_prev_finger_press_data.y_position,
#endif
ts->ts_data.curr_data[cnt].y_position)<10
)
#endif
							{
...
#if defined(CONFIG_MACH_MSM8960_FX1)
	ret += sprintf(buf+ret, "\tbutton_name           = %d, %d, %d\n", pdata->caps->button_name[0],
			pdata->caps->button_name[1], pdata->caps->button_name[2]);
#else
