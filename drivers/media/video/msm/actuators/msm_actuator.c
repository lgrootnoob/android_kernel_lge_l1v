...
#ifdef CONFIG_S5K4E1
		.actuator_set_default_focus = msm_actuator_s5k4e1_set_default_focus,
#else
...
#define S5K4E1_ACT_STOP_POS 10
int32_t msm_actuator_s5k4e1_set_default_focus(
	struct msm_actuator_ctrl_t *a_ctrl,
	struct msm_actuator_move_params_t *move_params)
{
	int32_t rc = 0;
	...
	if (a_ctrl->curr_step_pos != 0)
	{
		...
		uint32_t hw_damping = 0x0000000F;
		int udelay = 0;
		...
		int mid_dac = a_ctrl->step_position_table[S5K4E1_ACT_STOP_POS];
		pr_info("%s: called22\n",  __func__);

		if(cur_pos > S5K4E1_ACT_STOP_POS) {
			msm_actuator_i2c_write(a_ctrl, mid_dac, hw_damping);

//			udelay = (cur_dac - mid_dac) * 2 / 3;
			udelay = (cur_dac - mid_dac) * 200 / 2;
			CDBG("%s: To mid - step = %d, position = %d, delay = %d", __func__, cur_pos, cur_dac, udelay);
			pr_info("%s: To mid - step = %d, position = %d, delay = %d", __func__, cur_pos, cur_dac, udelay);
			usleep(udelay);//mdelay(delay);
		} else
			mid_dac = cur_dac;

		hw_damping = 0x0000000F;

		udelay = mid_dac  * 200 / 2;
		msm_actuator_i2c_write(a_ctrl, a_ctrl->initial_code + 10, hw_damping);

		CDBG("%s: To end - step = 10, position = %d, delay = %d", __func__, mid_dac, udelay);
		pr_info("%s: aaTo end - step = 10, position = %d, delay = %d", __func__, mid_dac, udelay);
		usleep(udelay);//mdelay(delay); // delay can be changed but put max due to it's small enough
		a_ctrl->curr_step_pos = 0;
...
int32_t msm_actuator_s5k4e1_power_down(struct msm_actuator_ctrl_t *a_ctrl)
{
	int32_t rc = 0;
...
//		delay = mid_dac / 2;
		...
		CDBG("%s: To end - step = 10, position = %d, delay = %d", __func__, mid_dac, udelay);
		pr_info("%s: To end - step = 10, position = %d, delay = %d", __func__, mid_dac, udelay);
		usleep(udelay);//mdelay(delay);	// delay can be changed but put max due to it's small enough
		a_ctrl->curr_step_pos = 0;
